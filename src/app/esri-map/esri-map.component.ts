...
export class EsriMapComponent implements OnInit, OnDestroy, OnChanges {
  ...
  ngOnInit() {
    ...
    this.initializeMap().then(mapView => {
      ...
      this.clickForInfo(this.fuelStationLayer);
      ...
      this._view.hitTest(event).then(response => {
        if(this.layerType === 'dPoint'){
          if (response.results.length) {
            console.log("Result: ",response);
            const graphic = response.results.filter(result => {
              return result.graphic.layer === inLayer;
            })[0].graphic;
            console.log("Station Name: " + graphic.attributes.Station_Name);
          }
        }
        if(this.layerType === 'county'){
          if (response.results.length) {
            ...
            console.log("Place Name: " + graphic.attributes.place_name);
          }
        }
      });
    });
  ...
  ngOnChanges(){
    ...
    if(this.layerType === 'county'){
      ...
      this.clickForInfo(this.graphicPointFeature);
      // this._view.watch("scale", newValue =>
      // {
